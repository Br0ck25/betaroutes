...
- [ ] Create a migration branch (**one theme per PR**)
  - [ ] Branch name example: `chore/svelte5-migrate-events` / `chore/svelte5-migrate-props`
- [ ] Confirm local Node version matches the repo requirement (Node `>= 22`)
- [ ] Run the baseline quality gate **before changes** (establish a "green" baseline)
  - [ ] `npm run gate` (typecheck + lint + unit tests)
...
- [ ] Review `svelte.config.*` and `vite.config.*` for Svelte 5 behavior changes
  - [ ] Remove or document deprecated/ignored compiler options (avoid "it worked in Svelte 4" assumptions)
  - [ ] If you relied on `accessors: true` or instance props, note that **runes mode ignores accessors**

### 0.2 Store-to-runes interop plan (if `svelte/store` remains)

- [ ] Decide the bridge strategy for any remaining `svelte/store` usage
  - [ ] Keep stores temporarily (no refactor in this PR)
  - [ ] Or migrate shared state into `.svelte.ts/.svelte.js` runes modules
- [ ] Enforce: **no new `svelte/store` usage** during the migration
