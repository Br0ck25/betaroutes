...
#log-list div[id^="details-"] {
  ...
  <body>
    ...
    <div
      ...
      "
    >
      ✅ You’re signed in as <span id="username-display"></span>
    ...
    <div id="log-container">
      <div id="log-header">
        ...
        <!-- ✅ Keep the filters next to the Log title -->
        <div
          id="log-title-bar"
          style="
            display: flex;
        ...
        >
          ...
          <!-- ✅ Replace your old filter div with this -->
    ...
    <script>
      ...
      document.addEventListener("DOMContentLoaded", async () => {
        ...
        displayLog();
        populateYearOptions(); // ✅ Rebuild years
        populateMonthOptions(); // 🆕 ✅ Rebuild months correctly based on logs
...
`;
  ...
  console.log("⏳ Saving to KV…", logEntries);
...
async function syncPendingLogs() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      await loadLog(); // ✅ Reload the latest logs
      displayLog();
    } else {
    ...
    console.error("❌ Error syncing pending logs:", err);
      ...
      async function loadLog() {
        ...
        if (!response.ok) {
          console.error("❌ Failed to load logs:", await response.text());
          return [];
        }
...
`;
  ...
  `;
      ...
      function populateYearOptions(preselectedYear = "") {
        ...
        // ✅ Preselect previously selected year if still exists
        ...
        // 🛠 DISABLE or HIDE year dropdown if only one real year
        if (years.length <= 1) {
      ...
      function populateMonthOptions(selectedYear = "", preselectedMonth = "") {
        ...
        // ✅ Re-select preselected month if still available
        if (
          preselectedMonth &&
        ...
        // 🛠 DISABLE if no months available (only "All")
    ...
    `;
      ...
      function saveEditedLogEntry() {
        ...
        // ✅ Grab destinations and earnings
        entry.destinations = [];
        entry.earnings = [];
        ...
        // 🆕 After populating years, also populate months correctly!
      ...
      function filterByDate() {
        ...
        populateMonthOptions(year, month); // 🆕 Repopulate month dropdown based on selected year!
      ...
      function handleTimeChange() {
        ...
        if (startTimeInput && endTimeInput) {
          ...
          hoursWorkedInput.readOnly = true; // 🛡️ make it readonly!
          hoursWorkedInput.style.backgroundColor = "#f0f0f0"; // light gray to indicate readonly
        } else {
          hoursWorkedInput.value = "";
          hoursWorkedInput.readOnly = false; // ✅ make it editable again
...
`;
      ...
      function recalculateEditHoursWorked() {
        ...
        if (startTime && endTime && hoursWorkedInput) {
          ...
          if (end < start) {
            // Assume the work ended next day
            end += 24 * 60;
          }
          ...
          hoursWorkedInput.value = workedHours.toFixed(2);
        }
      }
