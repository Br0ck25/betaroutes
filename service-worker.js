...
<html lang="en">
  <head>
    ...
    <script
    src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCHrNRHP9LNGVHi9Srk1Gfcrv-4ee_gUHQ&libraries=places&callback=initMap"
    defer
  ></script>
...
@media (max-width: 600px) {
  #top-bar {
    position: sticky;              /* ✅ preserve sticky on mobile */
    top: 0;
    z-index: 999;
    ...
    gap: 8px;
    margin: 0 auto;
    border-radius: 0;              /* ✅ matches desktop now */
    max-width: 100%;
  }
...
#logout-message img {
  ...
  align-items: center; /* 🛠 Center vertically with expanded details */
  padding: 10px 0;
...
.log-actions {
        ...
        min-width: 100px; /* 🆕 Keep actions a reasonable size */
        align-items: stretch; /* 🆕 Stretch buttons to same width */
      }
...
#log-container {
  ...
  width: auto;    /* 🆕 Remove 100% width */
      ...
      .log-actions button {
        ...
        color: white;
        border: none;
        padding: 8px 0; /* 🆕 Vertically thicker buttons */
        border-radius: 5px;
        cursor: pointer;
        font-size: 14px;
        text-align: center; /* Center button text */
        width: 100%; /* 🆕 Make each button full width of container */
      }
...
#log-list div[id^="details-"] {
  margin-left: 20px; /* ✅ Small indent for expanded text */
  padding-top: 10px;
...
<div style="margin-top: 10px;">
  <button id="hamburger-manage-account-toggle" onclick="toggleHamburgerManageAccountMenu()">
    Manage Account <span id="hamburger-manage-account-arrow" style="display: inline-block; transition: transform 0.3s;">▼</span>
  </button>
...
<button id="hamburger-manage-toggle" onclick="toggleHamburgerManageLogMenu()">
  Manage Log <span id="hamburger-manage-arrow" style="display: inline-block; transition: transform 0.3s;">▼</span>
</button>
...
">
  ...
  <button onclick="exportToPDFWithTotals()">Export PDF</button>
    ...
    </div> <!-- 👈 closes menu items -->
  </div> <!-- 👈 closes #account-menu -->
</div> <!-- 👈 closes #logout-message -->
    ...
    <div class="container" style="margin-top: 15px;">
      ...
      <label for="hours-worked">Total Hours</label>
...
<!-- 🆗 Android/Chrome will show this button; iOS won't -->
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (isIos()) {
    ...
    if (!isInStandaloneMode()) {
      ...
      // ⬇️ Insert below the log container
      ...
      // ⏱️ Auto-hide if user installs manually while the app is open
...
<div id="edit-hours-message" style="font-size: 14px; color: #666; margin-top: 4px;"></div>
    ...
    <script>
      ...
      document.addEventListener("click", function (event) {
        ...
        const isToggleButton =
          button && (button.innerText === "☰" || button.onclick === toggleMenu);
  ...
  if (!token) {
  console.error("❌ No token found. User must be signed in to save.");
  showAlertModal("❌ You must be signed in to save your route logs.", showLogin);
  return;
}
  ...
  showAlertModal("⚠️ Offline: Route saved locally. Will sync when back online.");
  return;
}
  ...
  try {
    ...
    localStorage.setItem("cachedLogs", JSON.stringify(logEntries)); // ✅ Save fresh logs locally
    localStorage.removeItem("pendingLogs"); // ✅ Clear pending if successful
  } catch (err) {
    console.error("❌ Save failed. Saving offline:", err);
    localStorage.setItem("pendingLogs", JSON.stringify(logEntries)); // ✅ Save pending
    localStorage.setItem("cachedLogs", JSON.stringify(logEntries)); // ✅ Update visible logs too
    showAlertModal("⚠️ Offline mode: Your changes are saved locally. We'll sync them next time you're online.");
  }
}
...
async function loadLog() {
  ...
  if (!token) {
    console.error("❌ No token found. Cannot load logs.");
    return [];
  }
...
const merged = logs.map(serverLog => {
  ...
  console.log("⛓ Conflict on", serverLog.date, "— using", serverTime >= localTime ? "server" : "local");
...
return merged;
    ...
    console.warn("⚠️ Offline or failed to load logs. Loading from local cache.", err);
    const cached = localStorage.getItem("cachedLogs");
...
function saveDraftTrip() {
  ...
  if (!hasMeaningfulData) {
    localStorage.removeItem("draftTrip"); // 🚫 don't store empty form
    return;
  }
...
document.getElementById("hours-worked").addEventListener("input", () => {
  ...
  const totalWorkedMinutes = Math.round((hoursWorked + driveTimeInHours) * 60);
  document.getElementById("total-hours").textContent = formatHoursAndMinutesFromMinutes(totalWorkedMinutes);

  // 👇 NEW: Display inside the input box itself
  document.getElementById("hours-worked").value = formatHoursAndMinutesFromMinutes(totalWorkedMinutes);
});
...
async function calculateRouteData() {
  ...
  let hoursWorked = parseFloat(document.getElementById("hours-worked").value) || 0;
  ...
  if (startTimeInput && endTimeInput) {
    ...
    document.getElementById("hours-worked").value = hoursWorked.toFixed(2);
    document.getElementById("hours-display").textContent = formatHoursAndMinutes(hoursWorked);
  }
    ...
    directionsService.route(request, (response, status) => {
      if (status === "OK") {
        ...
        if (startTimeInput && endTimeInput) {
          ...
          let totalWorkedMinutes = endTotalMinutes - startTotalMinutes;
          let driveMinutes = totalDuration / 60;
...
const driveTime = convertTimeToHours(data.totalTime || "");
const totalDecimal = hoursWorked + driveTime;

document.getElementById("hours-worked").value = formatHoursAndMinutes(data.hoursWorked ?? 0);
...
`;
  ...
  const token = localStorage.getItem("token");
  if (!token) {
    showAlertModal("❌ You must sign in to log your route.", showLogin);
    showLogin();
    return;
  }
...
async function optimizeRoute() {
  ...
  if (originalMileage === null) {
    await calculateRouteData(); // no alert or UI update
  }
  ...
  directionsService.route(request, (response, status) => {
    if (status === "OK" && response.routes[0].waypoint_order) {
      ...
      // ✅ Update map and show optimized results
      directionsRenderer.setDirections(response);
      ...
      // 🧠 Build optimizedData like your existing code does
      const optimizedData = {
        ...
        fuelCost: (
          (optimizedMileage / parseFloat(document.getElementById("mpg").value || 1)) *
      ...
      optimizedData.netProfit = (
        netProfitBeforeOptional - optimizedData.maintenanceCost - optimizedData.suppliesCost
      ).toFixed(2);

      const totalHoursSpent = optimizedData.hoursWorked + totalDuration / 3600;
      optimizedData.profitPerHour =
        totalHoursSpent > 0 ? (optimizedData.netProfit / totalHoursSpent).toFixed(2) : "0.00";

      updateUI(optimizedData);
      showConfirmationMessage("✅ Route optimized successfully!");
    } else if (status === "OK") {
      directionsRenderer.setDirections(response); // fallback
    } else {
...
`;
  ...
  data.hoursWorked = parseFloat(document.getElementById("hours-worked").value) || 0;
...
`;
  ...
  console.warn("❌ Missing or invalid start, end, or destination address — skipping route calculation.");
  return;
}
        ...
        directionsService.route(request, (response, status) => {
          if (status === "OK") {
            ...
            showAlertModal("⚠️ No route could be found between the entered locations. Please double-check the addresses.");
} else {
...
async function saveEditedLogEntry() {
  ...
  const totalHours = entry.hoursWorked + driveTime;

  entry.profitPerHour = totalHours > 0
    ? (netProfitFloat / totalHours).toFixed(2)
    : "0.00";
...
async function importLog() {
  ...
  fileReaderBusy = true; // ✅ block future reads until done
        ...
        if (line) {
          ...
          if (values && values.length === 13) {
            try {
              ...
              const destinations = values[2].slice(1, -1).split(";").map(d => d.replace(/""/g, '"'));
              const earnings = values[11].slice(1, -1).split(";").map(parseFloat);
              const totalTime = values[10].replace(/^"|"$/g, "").trim();
  ...
  totalTime: totalTime,
  earnings: earnings,
  profitPerHour: parseFloat(values[12]) || 0,
  mpg: 0, // optional default if missing
  gasPrice: 0,
  startClock: "", // could be parsed from time string if needed
  endClock: "",
  hoursWorked: 0,
  lastModified: new Date().toISOString(),
      ...
      await saveLog();
      filterLogs();
      showAlertModal("✅ Log imported successfully!");
    } catch (err) {
      console.error("❌ Error processing file:", err);
      showAlertModal("❌ Failed to process the file.");
    } finally {
      fileReaderBusy = false; // ✅ ready for next import
    }
  };
  ...
  showAlertModal(msg);
      ...
      function logout() {
        ...
        location.reload(); // ✅ this clears state and reinitializes
      }
      ...
      async function submitPasswordChange() {
        ...
        if (!current || !next || !confirm)
          return showAlertModal("⚠️ Please fill out all fields.");
  ...
  showAlertModal("❌ " + msg);
      ...
      async function submitResetPassword() {
        ...
        if (!username || !resetKey || !newPassword) {
          showAlertModal("⚠️ Please fill out all fields.");
          return;
        }
...
function recalculateEditHoursWorked() {
  ...
  hoursWorkedInput.value = formatHoursAndMinutes(decimalHours);

  document.getElementById("edit-hours-worked").value = formatHoursAndMinutes(entry.hoursWorked ?? 0);
...
function exportToCSVWithTotals() {
  ...
  const header =
    "Date,Start Time,End Time,Start Address,Destinations,End Address,Earnings per Stop,Mileage,Drive Time,Total Earnings,Fuel Cost,Maintenance,Supplies,Hours Worked,Total Hours,Net Profit,Profit Per Hour\n";
  ...
  const csvRows = filtered.map((entry) => {
    ...
    const totalHours = (parseFloat(entry.hoursWorked || 0) + driveMinutes / 60).toFixed(2);
  ...
  const totalHoursDecimal = totalSessionMinutes / 60;
  const profitPerHour = totalHoursDecimal > 0 ? totalNetProfit / totalHoursDecimal : 0;
...
function exportToPDFWithTotals() {
  ...
  const columns = [
    ...
    "Earnings per Stop", "Hours Worked", "Profit per Hour"
  ];
  ...
  const profitPerHour = totalSessionHours > 0 ? totalNetProfit / totalSessionHours : 0;
  ...
  if (accountMenu) {
    ...
    >
      <div onclick="event.stopPropagation()" class="modal-box">
        ...
        <p style="font-size: 14px; color: red">⚠️ This action is permanent.</p>
        <input
  type="password"
  id="delete-password"
