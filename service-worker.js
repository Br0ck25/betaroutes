...
<!-- 🔒 Only signed-in specific things -->
      ...
      ">
        ...
        <div style="display: flex; flex-direction: column; justify-content: center; align-items: center;">
          <button onclick="clearFilters()" style="
            ...
            margin-top: 34px; /* 🛠 Lifts it up slightly to match input perfectly */
          ">
            Clear
          </button>
        </div>
...
const logsPerPage = 10;
      ...
      function toggleManageLogMenu() {
        ...
        if (menu.style.display === "none" || menu.style.display === "") {
          ...
          body.style.paddingBottom = "200px"; // 🆕 Add extra bottom space
        } else {
          menu.style.display = "none";
          body.style.paddingBottom = "0"; // 🆕 Remove extra space
        }
      }
  ...
  if (!navigator.onLine) {
    ...
    alert("⚠️ Offline: Route saved locally. Will sync when back online.");
    return;
  }
  ...
  try {
    ...
    localStorage.setItem("cachedLogs", JSON.stringify(logEntries)); // ✅ Update visible logs too
    alert("⚠️ Offline mode: Your changes are saved locally. We'll sync them next time you're online.");
  }
}
...
async function loadLog() {
  ...
  try {
    ...
    localStorage.setItem("cachedLogs", JSON.stringify(logs)); // ✅ Save fresh logs locally
    return logs;
  } catch (err) {
          ...
          directionsService.route(request, (response, status) => {
            if (status === "OK") {
              ...
              if (startTimeInput && endTimeInput) {
                ...
                // ✅ Also auto-update the Hours Worked input
...
document.getElementById("total-hours").textContent = `${totalH} hours ${totalM} minutes`;
  ...
  const token = localStorage.getItem("token");
  if (!token) {
    alert("❌ You must sign in to log your route.");
    showLogin();
    return;
  }
  ...
  if (filledDestinations.length === 0) {
    alert("🚧 Please enter at least one destination before logging your route.");
    return;
  }
      ...
      async function optimizeRoute() {
        ...
        directionsService.route(request, (response, status) => {
          if (status === "OK" && response.routes[0].waypoint_order) {
            ...
            // ✅ Update map and show optimized results (WITHOUT overwriting original)
            directionsRenderer.setDirections(response);
...
`;
  ...
  `;
    ...
    `;
        ...
        // ✅ Only trigger recalculation when a valid place is selected
...
entry.gasPrice = getNumberValue("edit-gas-price");
  ...
  await saveLog(); // ✅ Wait for save
  filterLogs();

  closeEditForm(); // ✅ THEN close the edit screen
  showConfirmationMessage("✅ Log entry updated!");
      ...
      async function importLog() {
        ...
        if (!token) {
          alert("❌ You must be signed in to import a CSV file.");
...
await saveLog();        // ✅ Ensure logs are persisted
filterLogs();           // ✅ Refresh the UI
alert("Log imported successfully!");
      ...
      async function submitAuth() {
        ...
        if (res.ok) {
          ...
          // ✅ Show reset key if this was a signup
          ...
          // ✅ THEN reload the page after user has seen the key
          location.reload();
  ...
  if (token && username) {
    ...
    if (hamburgerButton) hamburgerButton.style.display = "inline-block"; // ✅ Show ☰
  } else {
    document.getElementById("auth-message").style.display = "block";
    document.getElementById("logout-message").style.display = "none";
    if (hamburgerButton) hamburgerButton.style.display = "none"; // ✅ Hide ☰
  }
}
...
function filterLogs() {
  ...
  entry?.date && filterFn(entry.date, entry)
        ...
        // ✅ Set the Net Profit and Profit Per Hour text ABOVE h2 (inside log-summary div)
...
let deferredPrompt; // ✅ Declare at top-level so all handlers can see it
