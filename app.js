/**
 * Go Route Yourself - Main Application JavaScript
 * This file contains ALL the original functionality plus enhancements
 */
...
<div id="offline-banner" style="
  ...
  font-size: 16px;
  max-width: 600px;
  margin: 120px auto 20px auto; /* ðŸ‘ˆ Increased top margin for signed-in layout */
  position: relative;
  z-index: 500;
">
...
<div id="logout-message" style="display: none; margin: 20px auto; max-width: 100%;">
  <div id="account-menu">
    ...
    <!-- ðŸ“‹ Menu Items -->
      ...
      <!-- âœ… New Manage Account collapsible section -->
<div style="margin-top: 10px;">
  <button id="hamburger-manage-account-toggle" onclick="toggleHamburgerManageAccountMenu()">
    Manage Account <span id="hamburger-manage-account-arrow" style="display: inline-block; transition: transform 0.3s;">â–¼</span>
  </button>
  <div id="hamburger-manage-account-menu" style="
    max-height: 0;
    overflow: hidden;
  ...
  ">
    ...
    <button onclick="closeMenu(); showDeleteModal()" class="danger">Delete Account</button>
  </div>
</div>


      <!-- âœ… NEW Manage Log button (collapsible) -->
<!-- Manage Log Toggle -->
<button id="hamburger-manage-toggle" onclick="toggleHamburgerManageLogMenu()">
  Manage Log <span id="hamburger-manage-arrow" style="display: inline-block; transition: transform 0.3s;">â–¼</span>
</button>


<!-- Dropdown Menu -->
<div id="hamburger-manage-log-menu" style="
  max-height: 0;
  overflow: hidden;
  transition: max-height 0.3s ease;
  background: white;
...
<!-- âœ… New Settings Dropdown -->
<!-- âœ… New Settings Dropdown -->
<button id="hamburger-settings-toggle" onclick="toggleHamburgerSettingsMenu()">
  Settings <span id="hamburger-settings-arrow" style="display: inline-block; transition: transform 0.3s;">â–¼</span>
</button>

<div id="hamburger-settings-menu" style="
  max-height: 0;
  overflow: hidden;
  transition: max-height 0.3s ease;
  background: white;
...
">
  ...
  <button onclick="window.open('/faq.html', '_blank')">FAQ</button>
    ...
    </div> <!-- ðŸ‘ˆ closes menu items -->
  </div> <!-- ðŸ‘ˆ closes #account-menu -->
</div> <!-- ðŸ‘ˆ closes #logout-message -->
...
<label for="log-notes" style="margin-top: 8px; display: block; font-weight: bold; color: #555;">Notes (Optional)</label>
<textarea
  id="log-notes"
  rows="4"
...
<div id="optimized-mileage-display" style="text-align: center; font-weight: bold; margin-top: 10px"></div>
    ...
    <div id="log-container">
      ...
      <!-- ðŸ†• Step 1: Centered Title and Summary -->
      ...
      <!-- ðŸ†• Step 2: Centered Filters Row -->
...
<div class="filter-section">
  ...
  <input type="text" id="filter-search" placeholder="Search anything..." oninput="filterLogs()" />
</div>
...
">
  <div class="modal-box" style="
    ...
    text-align: center;
  ">
    <h3>ðŸ“ Resume your last unsaved trip?</h3>
    <p style="margin-bottom: 20px;">We found a previous trip that wasn't saved. Would you like to continue editing it?</p>
    <button onclick="resumeDraftTrip()">Resume</button>
    <button onclick="discardDraftTrip()" style="margin-top: 10px; background-color: #ccc; color: black;">Discard</button>
  </div>
</div>
...
function clearTripForm() {
  ...
  // âŒ Clear only optional fields
  ...
  // âœ… Keep start, end, mpg, and gas-price
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (isIos()) {
    // âŒ Hide the install button on iOS
    if (installBtn) installBtn.style.display = 'none';

    // âœ… Only show iOS prompt if not installed
    if (!isInStandaloneMode()) {
      const prompt = document.createElement('div');
      prompt.id = 'ios-install-prompt'; // âœ… Needed for auto-hide
      prompt.innerHTML = `
        <div style="
          background-color: #fffae6;
          color: #333;
          ...
          max-width: 600px;
        ">
          ðŸ“± To install this app on your iPhone, tap the
          <strong>Share</strong> button
          <br>then choose <strong>Add to Home Screen</strong>.
        </div>
      `;

      // â¬‡ï¸ Insert below the log container
      ...
      // â±ï¸ Auto-hide if user installs manually while the app is open
...
function resumeDraftTrip() {
  ...
  // âœ… Restore logs first
  const cached = localStorage.getItem("cachedLogs");
  if (cached) {
    try {
      logEntries = JSON.parse(cached);
    } catch (e) {
      console.error("âŒ Failed to parse cachedLogs while resuming draft:", e);
    }
  }
  ...
  // âœ… Display restored logs at the end
  displayLog();
}
...
function discardDraftTrip() {
  ...
  if (cached) {
    try {
      ...
      console.error("âŒ Failed to parse cachedLogs:", e);
    }
  }
...
function isDuplicateEntry(entry, existingLog) {
  return existingLog.some((e) =>
    ...
    <div id="edit-form-container" class="edit-form-container hidden">
      ...
      <input type="number" id="edit-gas-price" placeholder="Enter Gas Price" oninput="updateEditFuelCost()" />
...
<label for="edit-notes" style="margin-top: 8px; display: block; font-weight: bold; color: #555;">Notes</label>
<textarea
  id="edit-notes"
  rows="4"
...
async function saveLog() {
  ...
  if (!navigator.onLine) {
    ...
    showAlertModal("âš ï¸ Offline: Route saved locally. Will sync when back online.");
    return;
  }
  ...
  try {
    ...
    // âœ… Force refresh after successful save
    await syncAndReloadLogs();

  } catch (err) {
    console.error("âŒ Save failed. Saving offline:", err);
    localStorage.setItem("pendingLogs", JSON.stringify(logEntries));
    localStorage.setItem("cachedLogs", JSON.stringify(logEntries));
    showAlertModal("âš ï¸ Offline mode: Your changes are saved locally. We'll sync them next time you're online.");
  }
}
...
async function syncPendingLogs() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.error("âŒ Failed to sync pending logs:", await response.text());
    }
  } catch (err) {
    console.error("âŒ Error syncing pending logs:", err);
  }
}
...
async function loadLog() {
  ...
  if (!token) {
    console.error("âŒ No token found. Cannot load logs.");
    return [];
  }
...
const merged = logs.map(serverLog => {
  ...
  console.log("â›“ Conflict on", serverLog.date, "â€” using", serverTime >= localTime ? "server" : "local");
...
async function syncAndReloadLogs() {
  if (skipNextSync) {
    console.log("â­ Skipping sync due to Cancel selection");
  ...
  try {
    ...
    console.error("âŒ Error refreshing logs:", error);
  }
}
...
function saveDraftTrip() {
  ...
  const notes = document.getElementById("log-notes")?.value.trim(); // âœ… add this
  ...
  const hasMeaningfulData =
    ...
    maintenance ||
    supplies ||
    notes; // âœ… now considers notes too
...
if (cached) {
  try {
    ...
    console.error("âŒ Failed to parse cachedLogs while showing resume modal:", e);
  }
}
...
return;
      ...
      console.error("âŒ Failed to parse draftTrip:", err);
      localStorage.removeItem("draftTrip");
    }
  }
...
if (dateInput) {
  ...
  dateInput.value = today; // âœ… set only the date
}
...
const totalWorkedMinutes = workedMinutes + driveMinutes;
  ...
  // ðŸ†• After selecting start time, auto-focus end time after slight delay
...
// ðŸ†• Ensure all logs have lastModified timestamp
...
async function calculateRoute() {
  ...
  if (filledDestinations.length === 0) {
    showAlertModal("ðŸš§ Please enter at least one destination before calculating your route.");
    return;
  }
...
async function calculateRouteData() {
  ...
  if (!navigator.onLine) {
    showAlertModal("ðŸ›œ You're offline. Route calculation is unavailable until you're reconnected.");
  ...
  return new Promise((resolve, reject) => {
    directionsService.route(request, (response, status) => {
      if (status === "OK") {
        ...
        showAlertModal("âŒ Directions request failed due to: " + status);
        document.getElementById("map").classList.add("hidden");
        document.getElementById("results").classList.add("hidden");
        resolve(null);
      }
...
`;
  ...
  // ðŸš¨ Check for at least one destination
  const destinationInputs = document.querySelectorAll('input[id^="destination-"]');
  const filledDestinations = Array.from(destinationInputs).filter(input => input.value.trim() !== "");
  if (filledDestinations.length === 0) {
    showAlertModal("ðŸš§ Please enter at least one destination before logging your route.");
    return;
  }

  // âœ… Get calculation result
  const calculationResult = await calculateRouteData();
  if (calculationResult) {
    currentPage = 1;
    ...
    displayLog();
    updateUI(calculationResult);
    clearTripForm(); // âœ… Reset the form
...
async function optimizeRoute() {
  ...
  if (currentDestinations.length < 2) {
    showAlertModal("ðŸš§ Please add at least two destinations to optimize the route.");
    return;
  }
  ...
  directionsService.route(request, (response, status) => {
    if (status === "OK" && response.routes[0].waypoint_order) {
      ...
      // âš¡ CALCULATE hoursWorked from start/end minus drive time
      ...
      const earnings = Array.from(document.querySelectorAll('input[id^="earnings-"]')).map(i => parseFloat(i.value) || 0);
...
// âœ… Scroll to map after optimization
document.getElementById("map")?.scrollIntoView({ behavior: "smooth" });

    } else if (status === "OK") {
      directionsRenderer.setDirections(response);
    } else {
      showAlertModal("âŒ Could not optimize route due to: " + status);
    }
  });
}
...
`;
  ...
  // âœ… Calculate total time from start and end clock times
  ...
  // âœ… Use total session time for profitPerHour
  const netProfitFloat = parseFloat(data.netProfit || 0);
  data.profitPerHour = totalHours > 0
    ? (netProfitFloat / totalHours).toFixed(2)
    : "0.00";
...
`;
  ...
  `;
      ...
      function goToPage(totalPages) {
        ...
        if (!isNaN(page) && page >= 1 && page <= totalPages) {
          ...
          showAlertModal("âŒ Invalid page number.");
        }
      }
  ...
  return logByDay;
      ...
      function recalculateEditMileageAndCosts() {
        ...
        const destinations = Array.from(destInputs)
          ...
          if (!start || !end || destinations.length !== destInputs.length || destinations.some(d => !d.location)) {
  console.warn("âŒ Missing or invalid start, end, or destination address â€” skipping route calculation.");
  return;
}
        ...
        directionsService.route(request, (response, status) => {
          if (status === "OK") {
            ...
            showAlertModal("âš ï¸ No route could be found between the entered locations. Please double-check the addresses.");
} else {
  ...
  if (prevDiv) {
    ...
    recalculateEditMileageAndCosts(); // âœ… added
  }
}
...
// âœ… Add listener to update total earnings
...
if (earningsInput) {
  ...
  if (destinationDiv) {
    ...
    renumberEditDestinations(); // ðŸ‘ˆ Use shared function
    updateEditTotalEarnings();  // ðŸ’° Optional if you're tracking per stop
    resetOriginalMileageDisplay();
    recalculateEditMileageAndCosts();
  }
}
      ...
      function openEditForm(index) {
        console.log('âœ… Opening edit for index:', index);
        editingIndex = index;
        const entry = logEntries[index];

        if (!entry) {
          console.error("âŒ No entry found for editing");
...
async function saveEditedLogEntry() {
  ...
  const netProfitFloat = entry.totalEarnings - entry.fuelCost - entry.maintenanceCost - entry.suppliesCost;

  const driveTime = convertTimeToHours(entry.totalTime || "");
// âœ… Use full clock span only (start-end time)
...
entry.profitPerHour = sessionHours > 0
  ? (netProfitFloat / sessionHours).toFixed(2)
  : "0.00";
  ...
  filterLogs();
  closeEditForm();
  showConfirmationMessage("âœ… Log entry updated!");
}
...
async function importLog() {
  if (fileReaderBusy) {
    console.warn("â³ FileReader is still busy. Try again in a moment.");
    return;
  }

  const token = localStorage.getItem("token");
  if (!token) {
    showAlertModal("âŒ You must be signed in to import a CSV file.", showLogin);
    return;
  }
...
for (const line of lines) {
  ...
  if (!values || values.length < 17) {
    console.warn("âŒ Skipping line â€” incorrect column count:", line);
    continue;
  }
  ...
  try {
    ...
    console.warn("âŒ Error parsing CSV row:", err, line);
  }
}
...
if (successCount === 0) {
  if (duplicateCount > 0) {
    showAlertModal("âš ï¸ All entries in the file were duplicates and have been skipped.");
  } else {
    showAlertModal("âŒ The file format appears to be invalid or unsupported.");
  }
  return;
}
      ...
      filterLogs();
      if (successCount > 0) {
  showAlertModal(`âœ… Imported ${successCount} entr${successCount === 1 ? "y" : "ies"} successfully.${duplicateCount > 0 ? `\nâš ï¸ Skipped ${duplicateCount} duplicate${duplicateCount === 1 ? "" : "s"}.` : ""}`);
} else {
  showAlertModal("âŒ The file format appears to be invalid or unsupported.");
}
      ...
      showAlertModal("âŒ Failed to process the file.");
    } finally {
...
reader.onerror = () => {
  ...
  fileReaderBusy = false;
  showAlertModal("âŒ Error reading the file.");
};
  ...
  if (!username || !password) {
    showAlertModal("âŒ Username and password required.");
    return;
  }
  ...
  if (res.ok) {
    ...
    if (isSignup && data.resetKey) {
      showAlertModal(
        `ðŸ” Your reset key is:<br><br><code style="font-size: 18px; user-select: all;">${data.resetKey}</code><br><br>Save this key somewhere safe.`,
        () => location.reload()
      );
    } else {
      // âœ… For login or failed signup-then-login, reload to reflect auth state
      location.reload();
    }
  } else {
  let msg = "âŒ An error occurred.";
  try {
    const contentType = res.headers.get("Content-Type") || "";
    if (contentType.includes("application/json")) {
      const data = await res.json();
      msg = "âŒ " + (data.error || msg);
    } else {
      msg = "âŒ " + (await res.text());
    }

    if (res.status === 400 && msg.toLowerCase().includes("username")) {
      msg = "âŒ That username is already taken. Please choose another.";
    }

    if (res.status === 404 && msg.toLowerCase().includes("not found")) {
      msg = "âŒ Account not found. Please check your username or sign up.";
    }
  ...
  showAlertModal(msg);
      ...
      async function submitPasswordChange() {
        ...
        if (!current || !next || !confirm)
          return showAlertModal("âš ï¸ Please fill out all fields.");

        if (next !== confirm) return showAlertModal("âŒ New passwords do not match.");

        const token = localStorage.getItem("token");
        const username = localStorage.getItem("username");

        if (!token || !username) {
  showAlertModal("âŒ You must be signed in.");
  return;
}
        ...
        if (res.ok) {
  showAlertModal("âœ… Password changed successfully.");
  closePasswordModal();
} else {
  const msg = await res.text();
  showAlertModal("âŒ " + msg);
}
      ...
      async function submitResetPassword() {
        ...
        if (!username || !resetKey || !newPassword) {
          showAlertModal("âš ï¸ Please fill out all fields.");
          return;
        }
        ...
        if (res.ok) {
  showAlertModal("âœ… Password reset successfully. You can now sign in.");
  closeResetModal();
} else {
  const msg = await res.text();
  showAlertModal("âŒ " + msg);
  ...
  if (!username || !token || !password) {
    showAlertModal("âŒ Missing credentials.");
    return;
  }
  ...
  if (res.ok) {
    ...
    // 6. Alert and reload
    showAlertModal("âœ… Your account has been deleted.", () => {
      location.reload();
    });
  } else {
    const msg = await res.text();
...
function filterLogs() {
  ...
  // âœ… Define the shared filter logic
  ...
  // âœ… Use that filter directly in display
...
function handleTimeChange() {
  ...
  // ðŸ†• Auto-open End Time picker after selecting Start Time
  if (startTimeInput && !endTimeInput) {
  ...
  if (startTimeInput && endTimeInput) {
    ...
    // âœ… Display in input as formatted string like "2 hours 15 minutes"
...
function clearFilters() {
  ...
  filterLogs(); // âœ… Refresh the display
}
...
function exportToCSVWithTotals() {
  const filtered = typeof currentFilterFn === "function"
    ? logEntries.filter((entry) => entry?.date && currentFilterFn(entry.date, entry))
    : logEntries;

  if (filtered.length === 0) {
    alert("âŒ No entries to export.");
    return;
  }

  const header =
    "Date,Start Time,End Time,Start Address,Destinations,End Address,Earnings per Stop,Mileage,Drive Time,Total Earnings,Fuel Cost,Maintenance,Supplies,Hours Worked,Total Hours,Net Profit,Profit Per Hour,Notes\n";
...
function exportToPDFWithTotals() {
  ...
  const columns = [
    "Date", "Start", "Destinations", "End",
    "Earnings", "Fuel", "Maintenance", "Supplies",
    "Net Profit", "Mileage", "Drive Time",
    "Earnings per Stop", "Hours Worked", "Total Hours", "Profit per Hour", "Notes"
  ];
...
async function handleGoogleSignIn(response) {
  ...
  try {
    ...
    if (data.token) {
      ...
      showConfirmationMessage("âœ… Signed in with Google!");
    } else {
...
<p style="text-align: center; margin-top: 10px">
  <a href="#" onclick="toggleAuthMode()">
    Switch to <span id="auth-switch-label">Sign Up</span>
  </a><br />
...
function formatHoursAndMinutes(decimal = 0) {
  ...
  const totalMinutes = Math.round(decimal * 60); // âœ… more accurate
  const hours = Math.floor(totalMinutes / 60);
  const minutes = totalMinutes % 60;
...
// âœ… Show install button if eligible and not already installed
...
// âœ… Handle click on install button
  ...
  if (deferredPrompt) {
    ...
    deferredPrompt.userChoice.then((choiceResult) => {
      if (choiceResult.outcome === "accepted") {
        ...
        console.log("âŒ User dismissed the install prompt");
      }
      deferredPrompt = null;
...
// âœ… Hide the install button if already installed (user is using PWA)
...
window.addEventListener('scroll', () => {
  ...
  async function onGoogleLogin(response) {
    try {
      ...
      alert("âŒ Google login failed: " + err.message);
    }
  }
...
function installApp() {
  if (deferredPrompt) {
    ...
    alert("âš ï¸ Installation is not supported or already installed.");
  }
}
