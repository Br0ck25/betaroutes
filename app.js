...
function openManageMaintenanceModal() {
  ...
  if (allCategories.length === 0) {
    showAlertModal("You don't have any maintenance categories. Click '+ Add Custom' to create one!");
    return;
  }
  
  const categoryList = allCategories.map(type => 
...
function deleteMaintenanceCategoryFromModal(typeName) {
  ...
  document.querySelectorAll(".maintenance-type").forEach(select => {
    ...
    const allCategories = getAllMaintenanceCategories();
    const categoryOptions = allCategories.map(type => `<option value="${type}">${type}</option>`).join("");
...
function addEditSupplyItem() {
  ...
  const customTypes = getCustomSupplyTypes();
  const customOptions = customTypes.map(type => `<option value="${type}">${type}</option>`).join("");
...
function getEditMaintenanceData() {
  const maintenance = [];
  const maintenanceItems = document.querySelectorAll("#edit-maintenance-container .maintenance-item");
  ...
  return existingLog.some((e) =>
    ...
    parseFloat(e.totalMileage) === parseFloat(entry.totalMileage)
      ...
      document.addEventListener("click", function (event) {
        ...
        const isHamburgerClick = hamburgerButton && 
...
async function saveLog() {
  ...
  console.log(" Sending to cloud:", logEntries.map(e => `${e.date} (${e.destinations?.length || 0} stops)`));
...
async function loadLog() {
  ...
  try {
    ...
    localTrips.forEach(localTrip => {
      const existsInCloud = cloudLogs.some(cloudTrip =>
    ...
    console.log(` Merge complete: ${cloudLogs.length} from cloud + ${addedCount} new local = ${merged.length} total`);
  ...
  return;
        ...
        directionsService.route(request, (response, status) => {
          if (status === "OK") {
            ...
            showAlertModal(" No route could be found between the entered locations. Please double-check the addresses.");
} else {
      ...
      function openEditForm(index) {
        ...
        if (entry.maintenance && Array.isArray(entry.maintenance)) {
          entry.maintenance.forEach(item => {
            ...
            if (presetTypes.includes(item.type) || customTypes.includes(item.type) || typeSelect.querySelector(`option[value="${item.type}"]`)) {
              typeSelect.value = item.type;
            } else {
              typeSelect.value = "Custom";
              customInput.style.display = "block";
  ...
  showAlertModal(msg);
      ...
      async function submitPasswordChange() {
        ...
        if (!current || !next || !confirm)
          return showAlertModal(" Please fill out all fields.");
...
async function updateSubscriptionInfoInMenu() {
  ...
  if (planNameEl) {
    planNameEl.textContent = subscription.plan.charAt(0).toUpperCase() + subscription.plan.slice(1) + " Plan";
  }
