...
test('Critical Path: Create and view a new trip', async ({ page }) => {
  ...
  try {
    ...
    // eslint-disable-next-line no-console
    console.log('NAV: fallback goto /dashboard/trips/new ->', { url: page.url(), closed: page.isClosed() });
  }
  ...
  try {
    ...
    // eslint-disable-next-line no-console
    console.log('Initial New Trip check failed, attempting reload and direct goto', { url: page.url(), closed: page.isClosed() });
    await page.reload();
    await page.goto('/dashboard/trips/new');
    await expect(page.locator('h1')).toContainText('New Trip', { timeout: 20000 });
    // eslint-disable-next-line no-console
    console.log('NAV: after reload goto /dashboard/trips/new ->', { url: page.url(), closed: page.isClosed() });
  }
  ...
  const dirRespPromise = page.waitForResponse((r) => r.url().includes('/api/directions/cache') && r.request().method() === 'GET');
  await dirRespPromise;
  // eslint-disable-next-line no-console
  console.log('DIRECTIONS: cache response received');
  // Capture input values to ensure bindings are correct
  // eslint-disable-next-line no-console
  console.log('INPUTS after fill:', await page.locator('#start-address').inputValue(), await page.locator('#end-address').inputValue());
  // eslint-disable-next-line no-console
  ...
  // eslint-disable-next-line no-console
  console.log('After selecting destination (place-selected), Trip Summary snapshot (truncated):', (await page.content()).slice(0, 500));
  // Debug: print the Trip Summary mileage displayed in the UI
  // eslint-disable-next-line no-console
  console.log('TripSummary distance displayed:', await page.locator('.text-xl.font-bold.text-green-900').first().textContent());
  try {
    await page.screenshot({ path: 'test-results/trip-flow-Critical-Path-Create-and-view-a-new-trip/after-destination.png' });
    // eslint-disable-next-line no-console
  ...
  try {
    await page.screenshot({ path: 'test-results/trip-flow-Critical-Path-Create-and-view-a-new-trip/address-filled.png' });
    // eslint-disable-next-line no-console
  ...
  // eslint-disable-next-line no-console
  console.log('STEP: clicked Continue -> awaiting Basic Information', { url: page.url(), closed: page.isClosed() });

  // Step 2: Basics (Date)
  // Wait for the Basic Information card and trip date input to be visible, then fill
  await page.locator('.card-title').filter({ hasText: 'Basic Information' }).waitFor({ state: 'visible', timeout: 10000 });
  await page.locator('#trip-date').waitFor({ state: 'visible', timeout: 5000 });
  await page.locator('#trip-date').fill('2025-01-01');
  await page.click('button:has-text("Continue")');
  // eslint-disable-next-line no-console
  console.log('STEP: clicked Continue from Basics -> awaiting Costs', { url: page.url(), closed: page.isClosed() });
  ...
  try {
    await page.screenshot({ path: 'test-results/trip-flow-Critical-Path-Create-and-view-a-new-trip/after-click-review.png' });
    // eslint-disable-next-line no-console
