<script lang="ts">
  ...
  onMount(async () => {
    ...
    if (API_KEY && !window.google) {
      ...
      script.async = true;
      script.onload = () => { mapLoaded = true; initMapServices(); };
  ...
  async function loadTripData() {
    ...
    if (tripData.startAddress) { setTimeout(() => calculateRoute(false), 500); }
    loading = false;
  }

  function initMap(node: HTMLElement) {
      if (!mapLoaded || !window.google) return;
      map = new google.maps.Map(node, { center: { lat: 37.7749, lng: -122.4194 }, zoom: 12, disableDefaultUI: true, zoomControl: true });
  ...
  $: {
      if (mapLoaded && step === 2) {
          void tripData.startAddress; void tripData.endAddress; void tripData.stops;
          clearTimeout(calcTimeout);
          calcTimeout = setTimeout(() => { if (tripData.startAddress) calculateRoute(false); }, 800);
      }
  }
  ...
  function calculateRoute(optimize = false) {
    ...
    if (map && directionsRenderer && directionsRenderer.getMap() !== map) { directionsRenderer.setMap(map); }
  ...
  function handleDrop(event: DragEvent, dropIndex: number) { event.preventDefault(); if (dragItemIndex === null) return; const item = tripData.stops[dragItemIndex]; const newStops = tripData.stops.filter((_, i) => i !== dragItemIndex); newStops.splice(dropIndex, 0, item); tripData.stops = newStops; dragItemIndex = null; calculateRoute(false); }

  function addStop() { if (!newStop.address) return; tripData.stops = [...tripData.stops, { ...newStop, id: crypto.randomUUID() }]; newStop = { address: '', earnings: 0, notes: '' }; }
  ...
  function formatTime12Hour(time24: string): string { if (!time24) return ''; const [hours, minutes] = time24.split(':'); const hour = parseInt(hours); const ampm = hour >= 12 ? 'PM' : 'AM'; const hour12 = hour % 12 || 12; return `${hour12}:${minutes} ${ampm}`; }
  function formatDateLocal(dateString: string): string { if (!dateString) return ''; const [year, month, day] = dateString.split('-').map(Number); const date = new Date(year, month - 1, day); return date.toLocaleDateString('en-US', { month: 'numeric', day: 'numeric', year: 'numeric' }); }
