<script lang="ts">
  ...
  // [!code ++] Dynamic Options from Settings
  ...
  // --- Helper: Get Local Date (Matches New Trip Page) ---
  ...
  async function loadTripData() {
    ...
    // Handle potential legacy field naming
    ...
    tripData = {
        ...
        suppliesItems: safeSupplies, // Normalized to suppliesItems for local state
        totalMiles: Number(found.totalMiles) || 0,
        mpg: Number(found.mpg) || $userSettings.defaultMPG || 25,
        gasPrice: Number(found.gasPrice) || $userSettings.defaultGasPrice || 3.50,
  ...
  let tripData = {
    ...
    suppliesItems: [] as any[], // Using this key to match New Trip
    notes: ''
  };
  ...
  async function handleStopChange(index: number, placeOrEvent: any) {
    const val = placeOrEvent?.formatted_address ||
        placeOrEvent?.name || tripData.stops[index].address;
    if (!val) return; 

    tripData.stops[index].address = val;
    isCalculating = true;
    try {
        const prevLoc = index === 0 ?
            tripData.startAddress : tripData.stops[index - 1].address;
        if (prevLoc) {
  ...
  async function handleMainAddressChange(type: 'start' | 'end', placeOrEvent: any) {
    const val = placeOrEvent?.formatted_address ||
        placeOrEvent?.name || (type === 'start' ? tripData.startAddress : tripData.endAddress);
  ...
  async function saveTrip() {
    ...
    const tripToSave = {
      ...
      // Map back supplyItems to both keys to ensure backend compatibility
    ...
    try { 
        ...
        toasts.error('Failed to update trip.');
    }
  }
      ...
      <div class="form-card">
        <div class="card-header">
          ...
          <button 
            ...
            type="button" 
            disabled={isCalculating || tripData.stops.length < 2}
            title="Reorder stops efficiently"
          >
