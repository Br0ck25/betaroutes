<script lang="ts">
  ...
  // [!code ++] Helper to traverse the entire list and update every leg
  // Essential for Drag & Drop or full re-calculations to fix "distanceFromPrev"
  ...
  async function handleStopChange(index: number, placeOrEvent: any) {
    ...
    if (!val) return; // Removed equality check to force update
  ...
  async function handleMainAddressChange(type: 'start' | 'end', placeOrEvent: any) {
    ...
    // Removed equality check to force update
  ...
  // [!code ++] New Handler for Auto-Add on Select
  ...
  async function addStop() {
    ...
    } catch (e) { alert("Error calculating route segment."); } finally { isCalculating = false; }
  }

  function removeStop(id: string) { 
      tripData.stops = tripData.stops.filter(s => s.id !== id); 
      // [!code changed] Use recalculateAllLegs to fix gap
      recalculateAllLegs(); 
  }
  ...
  // [!code changed] Use recalculateAllLegs
  ...
  async function saveTrip() {
    ...
    try { await trips.updateTrip(tripId, tripToSave, userId); alert('Trip updated successfully!'); goto('/dashboard/trips'); } catch (err) { alert('Failed to update trip.'); }
  }
