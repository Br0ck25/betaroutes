<script lang="ts">
  ...
  async function loadTripData() {
    ...
    // --- FIX: Ensure all arrays have IDs to prevent duplicates/crashes ---
  ...
  $: {
      if (mapLoaded && step === 2) {
          ...
          void tripData.endAddress; void tripData.stops;
  ...
  function handleDrop(event: DragEvent, dropIndex: number) { event.preventDefault(); if (dragItemIndex === null) return; const item = tripData.stops[dragItemIndex];
    const newStops = tripData.stops.filter((_, i) => i !== dragItemIndex); newStops.splice(dropIndex, 0, item); tripData.stops = newStops; dragItemIndex = null; calculateRoute(false);
  }
