<script lang="ts">
  ...
  onMount(async () => {
    ...
    // 2. Load Google Maps script securely
    if (!window.google) {
      ...
      script.onload = () => {
        ...
        // If data is ready, re-run initAutocomplete
        if (!loading) setTimeout(calculateRoute, 500);
      };
  ...
  // Form data (Matches New Trip Structure)
  let tripData = {
    id: tripId,
    date: '',
    startTime: '',
  ...
  async function loadTripData() {
    ...
    tripData = {
        ...
        startTime: trip.startTime || trip.startClock || '09:00',
        endTime: trip.endTime || trip.endClock || '17:00',
        hoursWorked: trip.hoursWorked || 0,
        startAddress: trip.startAddress || '',
        endAddress: trip.endAddress || '',
        stops: trip.destinations?.map((d: any) => ({
        ...
        estimatedTime: 0,
        mpg: trip.mpg || $userSettings.defaultMPG || 25,
        gasPrice: trip.gasPrice || $userSettings.defaultGasPrice || 3.50,
        fuelCost: trip.fuelCost || 0,
        maintenanceItems: trip.maintenanceItems?.map((i: any) => ({...i, id: i.id || crypto.randomUUID()})) || [],
        suppliesItems: trip.suppliesItems?.map((i: any) => ({...i, id: i.id || crypto.randomUUID()})) || [],
        notes: trip.notes || ''
    };
  ...
  function calculateRoute() {
    ...
    // Round trip logic: If end address is empty, use start address
    const destination = tripData.endAddress.trim() ? tripData.endAddress : tripData.startAddress;
    
    // Safety check: if pure round trip with no stops, maps might return 0
    if (destination === tripData.startAddress && waypoints.length === 0) return;
  ...
  async function updateTrip() {
    ...
    try {
      console.log('Updating trip:', tripData.id);
      await trips.updateTrip(tripData.id, tripToSave, userId);
      goto('/dashboard/trips');
    } catch (err) {
    ...
    <div class="form-content">
        ...
        <div class="form-card">
            ...
            {#if tripData.maintenanceItems.length > 0}
                ...
                {#each tripData.maintenanceItems as item (item.id)}
            ...
            {#if tripData.suppliesItems.length > 0}
                <div class="expense-items">
                {#each tripData.suppliesItems as item (item.id)}
                    <div class="expense-item">
                    <span class="expense-type">{item.type}</span>
                    <div class="expense-input">
                        <span class="currency">$</span>
        ...
        <div class="form-card">
            ...
            <button class="btn-primary" on:click={updateTrip} type="button">
                <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
