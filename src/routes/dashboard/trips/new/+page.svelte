      ...
      <div class="form-card">
        ...
        <div class="section-group">
            ...
            {#if showAddMaintenance}
                <div class="add-custom-row"><input type="text" bind:value={newMaintenanceItem} placeholder="Item name..." /><button class="btn-small primary" on:click={addCustomMaintenance}>Add</button></div>
            {/if}
        ...
        <div class="section-group">
            ...
            {#if showAddSupply}
                <div class="add-custom-row"><input type="text" bind:value={newSupplyItem} placeholder="Item name..." /><button class="btn-small primary" on:click={addCustomSupply}>Add</button></div>
            {/if}
            ...
            {#each tripData.suppliesItems as item}<div class="expense-row"><span class="name">{item.type}</span><div class="input-money-wrapper small"><span class="symbol">$</span><input type="number" bind:value={item.cost} placeholder="0.00" /></div><button class="btn-icon delete" on:click={() => removeSupplyItem(item.id)}>✕</button></div>{/each}
        </div>
    ...
    {#if step === 4}
      <div class="form-card">
        ...
        <div class="financial-summary">
            ...
            {#each tripData.suppliesItems as item}<div class="row detail"><span>{item.type}</span> <span class="val">{formatCurrency(item.cost)}</span></div>{/each}
        ...
        <div class="form-actions"><button class="btn-secondary" on:click={prevStep} type="button">Back</button><button class="btn-primary" on:click={saveTrip} type="button">Save Trip</button></div>
      </div>
    {/if}
  </div>
</div>
