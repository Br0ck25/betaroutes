<script lang="ts">
  ...
  let sortBy: 'date' | 'profit' | 'miles' = $state('date');
  let sortOrder: 'asc' | 'desc' = $state('desc');
  let filterProfit: 'all' | 'positive' | 'negative' = $state('all');
  ...
  // --- Filtering Logic ---
  const allFilteredTrips = $derived(
    $trips
      .filter((trip: Trip) => {
  ...
  const totalPages = $derived(Math.ceil(allFilteredTrips.length / itemsPerPage));
  const visibleTrips = $derived(
    allFilteredTrips.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage)
  );
  const allSelected = $derived(
    allFilteredTrips.length > 0 && selectedTrips.size === allFilteredTrips.length
  );
...
<AsyncErrorBoundary bind:this={tripsBoundary} onRetry={loadTrips}>
  <div class="trip-history">
    <div class="page-header">
      ...
      <div class="header-actions">
        <button class="btn-secondary" onclick={goToTrash} aria-label="View Trash">
          <svg
            width="20"
            height="20"
            viewBox="0 0 24 24"
        ...
        <button class="btn-secondary" onclick={openSettings} aria-label="Trip Settings">
          <svg
            width="20"
            height="20"
            viewBox="0 0 24 24"
      ...
      <div class="trip-list-cards">
        {#each visibleTrips as trip (trip.id)}
          <TripCard
            ...
            onToggleSelection={() => toggleSelection(trip.id)}
  ...
  {#snippet error({ error, retry }: { error: Error; retry: () => void })}
