<script lang="ts">
	...
	function parseGPX(gpxContent: string) {
		...
		// Basic parsing logic
		const startTimeElement = trkpts[0].querySelector('time')?.textContent;
		const endTimeElement = trkpts[trkpts.length - 1].querySelector('time')?.textContent;
		...
		previewTrip = {
			date: startTimeElement ? startTimeElement.split('T')[0] : new Date().toISOString().split('T')[0],
			startTime: startTimeElement ? startTimeElement.split('T')[1].slice(0,5) : '09:00',
			endTime: endTimeElement ? endTimeElement.split('T')[1].slice(0,5) : '17:00',
...
<div class="max-w-2xl mx-auto">
	...
	<p class="mb-6 text-gray-600">Upload a GPX file from your GPS device or other tracking apps to import your route data.</p>
		...
		{#if previewTrip}
			<div class="mt-6 border-t pt-4">
				...
				<div class="grid grid-cols-2 gap-4 text-sm mb-6">
					<div class="bg-gray-50 p-3 rounded"><span class="block text-xs text-gray-500 uppercase font-bold">Date</span> {previewTrip.date}</div>
					<div class="bg-gray-50 p-3 rounded"><span class="block text-xs text-gray-500 uppercase font-bold">Miles</span> {previewTrip.totalMiles}</div>
					<div class="bg-gray-50 p-3 rounded"><span class="block text-xs text-gray-500 uppercase font-bold">Start Time</span> {previewTrip.startTime}</div>
					<div class="bg-gray-50 p-3 rounded"><span class="block text-xs text-gray-500 uppercase font-bold">End Time</span> {previewTrip.endTime}</div>
				</div>
