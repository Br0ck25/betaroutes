<script lang="ts">
  ...
  // Configuration State
  let installPay: number = $state(0);
  let repairPay: number = $state(0);
  let upgradePay: number = $state(0);
  let wifiExtenderPay: number = $state(0);
  let voipPay: number = $state(0);
  let driveTimeBonus: number = $state(0);

  // Supply Costs
  let poleCost: number = $state(0);
  let concreteCost: number = $state(0);
  let poleCharge: number = $state(0);

  // Times (Default to standard times)
  let installTime: number = $state(90);
  let repairTime: number = $state(60);

  let overrideTimes = $state(false);

  let showSuccess = $state(false);
  let successMessage = $state('');
  let statusMessage = $state('Sync Now');
      ...
      {:else}
        ...
        <div class="warning-box">
          <p>
            ...
            <button type="button" class="btn-link" onclick={() => (showTripSettings = true)}>
              Trip Settings
            </button>
            .
          </p>
  ...
  <div class="modal-overlay">
    <div class="modal-content">
      ...
      <div class="modal-actions">
        <button class="btn-primary safe" onclick={cancelOverride}>
          <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"
            ><path
              stroke-linecap="round"
              stroke-linejoin="round"
