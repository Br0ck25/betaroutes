...
<script lang="ts">
  ...
  let taxYear = $state(SvelteDate.now().getFullYear()); // Default to current year
  let selectedTrips = $state(new SvelteSet<string>());
  let selectedExpenses = $state(new SvelteSet<string>());
  let selectedMileage = $state(new SvelteSet<string>());
  ...
  let dataType: 'trips' | 'expenses' | 'mileage' | 'tax-bundle' = $state('trips');
  // Date filters (YYYY-MM-DD)
  let dateFrom: string | undefined = $state();
  let dateTo: string | undefined = $state();
  // Export format
  ...
  // Pagination state
  let tripsDisplayCount = $state(10);
  let expensesDisplayCount = $state(10);
  let mileageDisplayCount = $state(10);
  ...
  let filteredExpenses = $derived(
    $expenses.filter((expense) => {
      ...
      // Extract YYYY-MM-DD for comparison (avoids timezone issues)
      const expenseDateStr = expense.date.includes('T')
  ...
  <div class="export-grid">
        ...
        {#if dataType === 'trips'}
          {#if filteredTrips.length > 0}
            ...
            {#if displayedTrips.length < filteredTrips.length}
              <button class="btn-load-more" onclick={loadMoreTrips}>
        ...
        {:else if dataType === 'expenses'}
          {#if filteredExpenses.length > 0}
            ...
            {#if displayedExpenses.length < filteredExpenses.length}
              <button class="btn-load-more" onclick={loadMoreExpenses}>
        ...
        {:else if dataType === 'mileage'}
          {#if filteredMileage.length > 0}
              ...
              {#each displayedMileage as mileageLog (mileageLog.id)}
                <label class="trip-checkbox">
                  <input
                    ...
                    onchange={() => toggleMileage(mileageLog.id)}
                  />
                  <div class="trip-info">
                    <div class="trip-header">
            ...
            {#if displayedMileage.length < filteredMileage.length}
              <button class="btn-load-more" onclick={loadMoreMileage}>
    ...
    <div class="options-card">
      ...
      {#if showImportPreview}
        <div class="import-preview">
          ...
          <button class="btn-export" onclick={confirmImport}>Confirm Import</button>
          <button class="btn-export secondary" onclick={() => (showImportPreview = false)}
            >Cancel</button
          >
        </div>
      {/if}
    ...
    <div class="options-card">
      ...
      <div class="option-group">
        <button class="export-btn full-width" onclick={backupFullData}>
          <svg
            width="20"
            height="20"
            viewBox="0 0 24 24"
    ...
    <div class="options-card danger-card">
      ...
      <div class="option-group">
        <button class="export-btn danger full-width" onclick={confirmClearData}>
        ...
        <div class="confirm-modal">
          <div class="modal-content">
            ...
            <div class="modal-actions">
              <button class="export-btn danger" onclick={clearLocalData}
                >Yes, Delete Everything</button
              >
              <button class="export-btn secondary" onclick={() => (showClearConfirm = false)}
                >Cancel</button
              >
            </div>
          </div>
