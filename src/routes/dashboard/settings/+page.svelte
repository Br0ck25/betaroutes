<script lang="ts">
  ...
  // 1. Initialize local form state from store (defaults)
  let settings = { ...$userSettings };

  // 2. If remote settings exist (loaded by server), merge them in
  // This ensures that when you switch browsers, the server data takes precedence
  $: if (data.remoteSettings?.settings) {
    ...
    // Update the local form variable
    settings = merged;
  }
  ...
  // We prioritize the remote saved data if available
