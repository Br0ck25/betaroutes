<script lang="ts">
  ...
  // Helper to parse item strings "Name:50.00 | Item2:20.00"
  ...
  function exportCSV() {
    ...
    const headers = [
      ...
      'Maintenance Cost', 'Maintenance Items', // Added Items Column
      'Supply Cost', 'Supply Items',           // Added Items Column
    ...
    input.onchange = async (e: any) => {
      ...
      try {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          // MAPPING (New indices because we added 2 columns)
          // 0: Date
          // 1: Start
          // 2: Stop Addrs
          // 3: End
          ...
          const suppliesCost = parseFloat(cleanRow[13]) || 0; // Index shifted to 13

          // --- MAINTENANCE ITEMS LOGIC ---
          let maintenanceItems = parseItemString(cleanRow[12]);
          // Fallback for old CSVs or manual edits where only total is present
          ...
          parsed.push({
            ...
            notes: cleanRow[17] || '', // Index shifted to 17
            startTime: '09:00',
            endTime: '17:00',
            mpg: 25,
            gasPrice: 3.50,
      ...
      <div class="card-header">
        <div class="card-icon orange">
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
            ...
            <path d="M10 12C4.47715 12 0 15.3579 0 19.5C0 19.7761 0.223858 20 0.5 20H19.5C19.7761 20 20 19.7761 20 
19.5C20 15.3579 15.5228 12 10 12Z" fill="currentColor"/>
          </svg>
        </div>
        <div>
      ...
      <div class="card-header">
        <div class="card-icon green">
           <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
            ...
            <path d="M16.2 12C16.1 12.5 16.3 13 16.7 13.3L16.8 13.4C17.1 13.7 17.3 14.1 17.3 14.5C17.3 14.9 17.1 15.3 16.8 15.6C16.5 15.9 16.1 16.1 15.7 16.1C15.3 16.1 14.9 15.9 14.6 15.6L14.5 15.5C14.2 15.1 13.7 14.9 13.2 15C12.7 15.1 12.4 15.5 12.3 
16V16.2C12.3 17.1 11.6 17.8 10.7 17.8C9.8 17.8 9.1 17.1 9.1 16.2V16.1C9 15.5 8.6 15.1 8 15C7.5 15 7 15.2 6.7 15.6L6.6 15.7C6.3 16 5.9 16.2 5.5 16.2C5.1 16.2 4.7 16 4.4 15.7C4.1 15.4 3.9 15 3.9 14.6C3.9 14.2 4.1 13.8 4.4 13.5L4.5 13.4C4.9 13.1 5.1 12.6 5 12.1C4.9 11.6 4.5 11.3 4 11.2H3.8C2.9 11.2 2.2 10.5 2.2 9.6C2.2 8.7 2.9 8 3.8 8H3.9C4.5 7.9 4.9 7.5 5 6.9C5 6.4 4.8 5.9 4.4 5.6L4.3 5.5C4 5.2 3.8 4.8 3.8 4.4C3.8 4 4 3.6 4.3 3.3C4.6 3 5 2.8 5.4 2.8C5.8 2.8 6.2 3 6.5 3.3L6.6 3.4C7 3.8 7.5 4 8 3.9C8.5 
    ...
    <div class="settings-card">
      <div class="card-header">
        <div class="card-icon navy">
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
            <path d="M17 2H3C2.46957 2 1.96086 2.21071 1.58579 2.58579C1.21071 2.96086 1 3.46957 1 4V16C1 16.5304 1.21071 17.0391 1.58579 17.4142C1.96086 17.7893 2.46957 18 
3 18H17C17.5304 18 18.0391 17.7893 18.4142 17.4142C18.7893 17.0391 19 16.5304 19 16V4C19 3.46957 18.7893 2.96086 18.4142 2.58579C18.0391 2.21071 17.5304 2 17 2Z" stroke="currentColor" stroke-width="2"/>
    ...
    <div class="settings-card danger-card">
      <div class="card-header">
        <div class="card-icon red">
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
            <path d="M7 17H3C2.46957 17 1.96086 16.7893 
1.58579 16.4142C1.21071 16.0391 1 15.5304 1 15V3C1 2.46957 1.21071 1.96086 1.58579 1.58579C1.96086 1.21071 2.46957 1 3 1H7M13 13L17 9M17 9L13 5M17 9H7" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
          </svg>
        </div>
        <div>
...
deletePassword = ''; deleteError = ''; }}>Cancel</button>
            </div>
          </div>
        {/if}
      </div>
...
padding: 14px 20px; border-radius: 12px; font-size: 14px; font-weight: 500; margin-bottom: 24px; }
...
border: 1px solid #BBF7D0; }
  ...
  .settings-card { background: white;
...
border-radius: 10px; font-size: 15px; font-family: inherit; background: white; transition: all 0.2s; 
    display: block;
/* Ensures select elements behave as blocks */
...
width: 100%;
  ...
  /* Plan & Usage */
  .divider { height: 1px;
background: #E5E7EB; margin: 24px 0; }
...
color: #DC2626; border: none; margin-top: 12px; font-size: 14px; text-decoration: underline; }
  ...
  .delete-input { width: 100%; padding: 10px;
border: 1px solid #E5E7EB; border-radius: 8px; margin-bottom: 12px; }
  ...
  .action-btn { display: flex; align-items: center; gap: 16px; padding: 16px; background: #F9FAFB; border: 2px solid #E5E7EB; border-radius: 12px;
