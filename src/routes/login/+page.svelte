<script lang="ts">
    ...
    // Removed: enhance, ActionData (No longer using form actions)

    // --- State Management ---
    // Form Data - Renamed to match the server-side logic
    let username = '';      // Used as 'identifier' for login, and 'username' for register
    let email = '';         // Only used for registration
    let password = '';
    let confirmPassword = '';
    ...
    // CRITICAL FIX: Determine mode based on URL, ensuring it's reactive
    $: isLogin = $page.url.searchParams.get('view') !== 'register';

    /**
     * Function to toggle mode and update the URL query parameter.
     * This ensures the mode persists if the user refreshes the page.
     */
    function toggleMode() {
        ...
        // Clear form inputs and messages when switching modes
        username = '';
        email = '';
        password = '';
        confirmPassword = '';
    ...
    /**
     * CRITICAL FIX: Submits form via fetch/JSON to the +server.ts endpoint.
     */
    async function submitHandler() {
        ...
        // --- 1. Client-Side Validation & Payload Generation ---
        if (isLogin) {
            if (!username || !password) {
                responseError = 'Please enter your username/email and password.';
                loading = false;
                return;
            }
            ...
            // Register: Server expects { username, email, password }
        ...
        try {
            ...
            if (response.ok) {
                // Success: The server set the 'token' cookie. Redirect to dashboard.
                await goto('/dashboard', { invalidateAll: true });
            } else {
                // Failure: Display error message from the server response
                responseError = result.message || 'An unknown error occurred during sign-in.';
            }
        } catch (e) {
            console.error('Fetch error:', e);
            responseError = 'A network or server error occurred. Check console for details.';
        } finally {
            loading = false;
        }
    }
...
<style>
	...
	* {
	...
	.alert.success {
		/* This block is now unused as success redirects */
		background: #F0FDF4;
		color: #166534;
	}
