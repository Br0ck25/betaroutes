...
// ✅ CORRECTED: Adds userId to sync queue for user scoping
...
class SyncManager {
   ...
   * ✅ SECURITY FIX: Requires userId parameter
   */
  async addToQueue(
    item: Omit<SyncQueueItem, 'id' | 'timestamp' | 'retries' | 'userId'> & { userId?: string }
  ) {
    // Extract userId from item if present, otherwise throw
    const userId = item.userId;
    if (!userId) {
      console.error('❌ userId required for addToQueue');
      throw new Error('userId required for sync queue');
    }
    ...
    await tx.objectStore('syncQueue').add({
      ...item,
      userId, // ✅ Add userId to queue item
      timestamp: Date.now(),
      retries: 0
    });
    await tx.done;
    ...
    if (
      ...
      try {
        try {
          ...
          console.warn('⚠️ Google Maps failed to load. Skipping.', loaderErr);
          return;
        }
