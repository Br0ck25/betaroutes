...
import { 
    parseDateOnly, toIsoDate, extractDateFromTs, formatTimestamp, validateSyncConfig, 
    isValidAddress, determineOrderSyncStatus, checkIncompleteToComplete 
} from './utils';
...
export class HughesNetService {
    ...
    private async acquireLock(lockKey: string, ownerId: string): Promise<boolean> {
        ...
        try {
            await this.kv.put(lockKey, JSON.stringify(lock), { expirationTtl: Math.ceil(LOCK_TTL_MS / 1000) });
    ...
    private async waitForLock(lockKey: string, ownerId: string): Promise<boolean> {
        for (let i = 0; i < LOCK_MAX_RETRIES; i++) {
            ...
            } catch (e) { console.warn('Failed to check lock expiry:', e); }
    ...
    async saveSettings(userId: string, settings: any) {
        try {
            ...
            if (settings.installTime < 0 || settings.repairTime < 0) throw new Error("Job times cannot be negative");
    ...
    async sync(
        userId: string, settingsId: string | undefined, installPay: number, repairPay: number, upgradePay: number, 
        poleCost: number, concreteCost: number, poleCharge: number, wifiExtenderPay: number, voipPay: number, 
        driveTimeBonus: number, skipScan: boolean, recentOnly: boolean = false, forceDates: string[] = []
    ): Promise<SyncResult> {
        ...
        try {
            ...
            if (!lockAcquired) throw new Error('Could not acquire sync lock. Another sync may be in progress.');
            const result = await this.performSync(
                userId, settingsId, installPay, repairPay, upgradePay, 
                poleCost, concreteCost, poleCharge, wifiExtenderPay, voipPay, 
                driveTimeBonus, skipScan, recentOnly, forceDates
            );
            return result;
        } finally {
    ...
    private async performSync(
        userId: string, settingsId: string | undefined, installPay: number, repairPay: number, upgradePay: number, 
        poleCost: number, concreteCost: number, poleCharge: number, wifiExtenderPay: number, voipPay: number, 
        driveTimeBonus: number, skipScan: boolean, recentOnly: boolean, forceDates: string[]
    ): Promise<SyncResult> {
        ...
        else this.warn(`[Sync] Order database too large (${(dbString.length / 1024 / 1024).toFixed(2)}MB), rollback disabled`);
        ...
        try {
            ...
            if (!skipScan) {
                ...
                try {
                    ...
                    links.push({ url: 'https://dwayinstalls.hns.com/CROF/SoSearch.jsp?action=submit', text: 'Manual Search' });
                    ...
                    for (const link of links) {
                        ...
                        try {
                            ...
                            if (e.message === 'REQ_LIMIT') { this.warn('[Scan] Hard limit reached'); break; }
                            this.warn(`[Scan] Failed to scan ${link.url}: ${e.message}`);
                        }
                    }
                } catch (e: any) {
                    if (e.message === 'REQ_LIMIT') this.warn('[Scan] Hard limit reached during initial scan');
                    else if (e.message !== 'Session expired.') this.error('[Scan] Error', e);
                    else throw e;
                }
                if (dbDirty) { await this.kv.put(`hns:db:${userId}`, JSON.stringify(orderDb)); dbDirty = false; }
                if (this.fetcher.shouldBatch()) {
                ...
                try {
                    ...
                    if (!this.fetcher.shouldBatch() && !recentOnly) {
                        ...
                        while(failures < DISCOVERY_MAX_FAILURES && checks < DISCOVERY_MAX_CHECKS) { 
                              if (this.fetcher.shouldBatch()) { this.log('[Discovery] Soft limit reached during backward scan'); break; }
                    ...
                    if (e.message === 'REQ_LIMIT') this.warn('[Discovery] Hard limit reached');
                    else this.error('[Discovery] Unexpected error', e);
                }
                if (dbDirty) { await this.kv.put(`hns:db:${userId}`, JSON.stringify(orderDb)); dbDirty = false; }
                if (this.fetcher.shouldBatch()) {
            ...
            if (missingDataIds.length > 0) {
                ...
                try { cookie = await this.refreshSessionIfNeeded(userId); } 
                catch (e: any) { this.error('[Download] Session refresh failed', e); throw e; }
                ...
                for (const id of missingDataIds) {
                    ...
                    try {
                        ...
                        if (e.message === 'REQ_LIMIT') { this.warn('[Download] Hard limit reached'); incomplete = true; break; }
                        if (e.message.includes('Session expired')) throw e;
    ...
    private groupOrdersByDate(orderDb: Record<string, OrderData>): Record<string, OrderData[]> {
        ...
        for (const order of Object.values(orderDb)) {
            if (!isValidAddress(order)) { this.warn(`[Trips] Skipping order ${order.id} - invalid address`); continue; }
    ...
    private async processOrderData(orderDb: Record<string, OrderData>, id: string, parsed: OrderData, ownerId?: string): Promise<boolean> {
        ...
        delete parsed._status;
        const updatedOrder: OrderData = { ...parsed, syncStatus, needsResync, lastSyncTimestamp: Date.now() };
        
        if (wasIncompleteNowComplete) { updatedOrder.lastPaymentUpdate = Date.now(); this.log(`  ${id} [INCOMPLETE→COMPLETE] Payment update needed`); }
