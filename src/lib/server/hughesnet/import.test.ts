...
describe('HughesNet archived import API', () => {
	...
	it('imports selected archived orders into user hns db', async () => {
		const kv = (platform as any).env['BETA_HUGHESNET_ORDERS_KV'] as any;
		const hns = (platform as any).env['BETA_HUGHESNET_KV'] as any;
		const owner = 'import_user';
		const orderId = '9001';
		const wrapper = {
			ownerId: owner,
			storedAt: Date.now(),
			order: { id: orderId, address: '9001 Test Pl', confirmScheduleDate: '12/31/2025' }
		};
		...
		const event: any = {
			platform,
			locals: { user: { name: owner } },
			request: makeReq({ ids: [orderId] })
		};
	...
	it('skips orders not owned by user', async () => {
		...
		const wrapper = {
			ownerId: other,
			storedAt: Date.now(),
			order: { id: orderId, address: '9002 Place' }
		};
		await kv.put(`hns:order:${orderId}`, JSON.stringify(wrapper));
