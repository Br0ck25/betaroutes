...
const mockDB: Record<string, Record<string, unknown>> = {
  ...
  HUGHESNET_ORDERS: {},
  PLACES: {},
  INDEXES: {} // Storage for Trip Indexes
};
...
async function initMockDB() {
  ...
  try {
      ...
      if (parsed && typeof parsed === 'object') {
        ...
        for (const [k, v] of Object.entries(loaded)) {
          if (v && typeof v === 'object') {
            mockDB[k] = v as Record<string, unknown>;
          }
        }
      }

      // Ensure namespaces exist
      for (const ns of ['HUGHESNET', 'HUGHESNET_ORDERS', 'PLACES', 'SESSIONS', 'INDEXES']) {
    ...
    log.error('Failed to load mock DB', { error: e instanceof Error ? e.message : String(e) });
  }
}
...
async function saveDB() {
  try {
    ...
    log.error('Failed to save mock DB', { error: e instanceof Error ? e.message : String(e) });
  }
}

type MockKVListResult = { keys: Array<{ name: string }> };
type MockKVNamespace = {
  get: (key: string) => Promise<string | null>;
  put: (key: string, value: string) => Promise<void>;
  delete: (key: string) => Promise<void>;
  list: (opts?: { prefix?: string }) => Promise<MockKVListResult>;
};

// Factory function for KV
function createMockKV(namespace: string): MockKVNamespace {
  return {
...
type BillingStore = Record<string, number>;
type IndexStorage = { trips: unknown[]; initialized: boolean; billing: BillingStore };

// Factory function for Durable Object Stub
function createMockDOStub(id: string) {
  return {
    async fetch(urlOrRequest: string | Request, init?: RequestInit) {
      ...
      const storage = mockDB['INDEXES'][id] as IndexStorage;
      ...
      if (path === '/migrate') {
        ...
        storage.trips = Array.isArray(body) ? body : [];
      ...
      if (path === '/put') {
        const trip = JSON.parse((init?.body as string) || 'null') as unknown;
        const tripId =
          trip && typeof trip === 'object'
            ? ((trip as Record<string, unknown>)['id'] as unknown)
            : undefined;

        if (typeof tripId !== 'string') return new Response('Bad Request', { status: 400 });

        const idx = storage.trips.findIndex((t) => {
          if (!t || typeof t !== 'object') return false;
          return (t as Record<string, unknown>)['id'] === tripId;
        });
        ...
        // Safe Sort desc
        storage.trips.sort((a, b) => getSortValue(b).localeCompare(getSortValue(a)));
        await saveDB();
        return new Response('OK');
      }

      if (path === '/delete') {
        const body = JSON.parse((init?.body as string) || 'null') as unknown;
        const tripId =
          body && typeof body === 'object'
            ? ((body as Record<string, unknown>)['id'] as unknown)
            : undefined;

        if (typeof tripId !== 'string') return new Response('Bad Request', { status: 400 });

        storage.trips = storage.trips.filter((t) => {
      ...
      if (path === '/billing/check-increment') {
        ...
        const monthKey =
          body && typeof body === 'object'
            ? ((body as Record<string, unknown>)['monthKey'] as unknown)
            : undefined;
        const limit =
          body && typeof body === 'object'
            ? ((body as Record<string, unknown>)['limit'] as unknown)
            : undefined;

        if (typeof monthKey !== 'string' || typeof limit !== 'number') {
      ...
      if (path === '/billing/decrement') {
        ...
        if (typeof monthKey !== 'string') return new Response('Bad Request', { status: 400 });
 ...
 * Main Setup Function
 */
export async function setupMockKV(event: MockableEvent) {
