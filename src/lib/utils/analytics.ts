...
export interface PeriodBreakdown {
  ...
  monthly: Array<{ month: string; revenue: number; expenses: number; profit: number; trips: number }>;
  quarterly: Array<{ quarter: string; revenue: number; expenses: number; profit: number; trips: number }>;
}

/**
 * Calculate comprehensive analytics from trip data
 */
export function calculateTripAnalytics(trips: any[]): TripAnalytics {
  if (trips.length === 0) {
    return {
      totalRevenue: 0,
      avgRevenuePerTrip: 0,
  ...
  trips.forEach(trip => {
    // Revenue from stops
    const tripRevenue = trip.stops?.reduce((sum: number, stop: any) => 
      sum + (stop.earnings || 0), 0) || 0;
 ...
 */
export function calculateMonthlyComparison(allTrips: any[]): MonthlyComparison {
  const now = new Date();
 ...
 */
export function calculatePeriodBreakdown(trips: any[]): PeriodBreakdown {
  ...
  return {
    weekly: Object.values(weeklyData).sort((a: any, b: any) => a.week.localeCompare(b.week)),
    monthly: Object.values(monthlyData).sort((a: any, b: any) => a.month.localeCompare(b.month)),
    quarterly: Object.values(quarterlyData).sort((a: any, b: any) => a.quarter.localeCompare(b.quarter))
  };
}
