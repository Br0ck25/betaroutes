...
function resumeDraftTrip() {
  ...
  document.getElementById("total-hours").value = formatHoursAndMinutes(
    data.hoursWorked || 0
  );
  ...
  data.destinations.forEach((d, i) => {
    ...
    div.innerHTML = `
      ...
      <input type="text" id="destination-${
        i + 1
      }" list="recent-destinations" value="${d.address || ""}">
...
function showConfirmModal(message, onConfirm, onCancel) {
  ...
  buttons.innerHTML = `
    ...
    <button onclick="closeUniversalModal(); ${
      onCancel ? "window._universalCancelCallback();" : ""
    }">No</button>
  `;
...
async function saveLog() {
  ...
  if (!token) {
    ...
    showAlertModal(
      "❌ You must be signed in to save your route logs.",
      showLogin
    );
    return;
  }
...
async function syncPendingLogs() {
  ...
  try {
    ...
    console.warn("❌ Invalid pending logs, clearing:", err.message);
    localStorage.removeItem("pendingLogs"); // ❌ purge bad data
    return;
  }
  ...
  if (res.ok) {
    console.log("✅ Synced pending logs");
...
async function loadLog() {
  ...
  try {
    ...
    try {
      ...
      throw new Error("❌ Invalid JSON in logs response: " + parseErr.message);
    }

    const localLogs = JSON.parse(localStorage.getItem("cachedLogs") || "[]");

    const merged = logs.map((serverLog) => {
      const localMatch = localLogs.find(
        (l) =>
          l.date === serverLog.date &&
          JSON.stringify(l.destinations) ===
            JSON.stringify(serverLog.destinations)
      );
    ...
    console.warn(
      "⚠️ Offline or failed to load logs. Loading from local cache. Error:",
      err.message
    );
    const cached = localStorage.getItem("cachedLogs");
    try {
      ...
      console.error("❌ Failed to parse cached logs:", parseErr.message);
      return [];
    }
  }
}
...
function saveDraftTrip() {
  ...
  const destinations = Array.from(
    ...
    .map((div) => ({
      address:
        div.querySelector('input[id^="destination-"]')?.value.trim() || "",
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
      ...
      if (cached) {
        try {
          ...
          console.error(
            "❌ Failed to parse cachedLogs while showing resume modal:",
            e
          );
        }
      }
      ...
      destinations.forEach((d, i) => {
        ...
        div.innerHTML = `
          ...
          <label for="earnings-${i + 1}">Earnings for Destination ${
          i + 1
        }</label>
          <input type="number" id="earnings-${i + 1}" value="${
          d.earnings || ""
        }">
          <div class="destination-actions">
            <button class="delete-btn" onclick="deleteDestination(this)">Delete</button>
  ...
  document.getElementById("total-hours").addEventListener("input", () => {
    const hoursWorked =
      parseFloat(document.getElementById("total-hours").value) || 0;

    const driveTimeInHours = convertTimeToHours(
...
async function calculateRoute() {
  ...
  if (filledDestinations.length === 0) {
    showAlertModal(
      "🚧 Please enter at least one destination before calculating your route."
    );
    return;
  }
...
async function calculateRouteData() {
  ...
  if (startTimeInput && endTimeInput) {
    ...
    document.getElementById("total-hours").value =
      formatHoursAndMinutes(hoursWorked);
  }
  ...
  if (!navigator.onLine) {
    showAlertModal(
      "🛜 You're offline. Route calculation is unavailable until you're reconnected."
    );
...
function updateUI(data) {
  if (data) {
    ...
    const minuteText =
      totalM > 0 ? ` ${totalM} minute${totalM !== 1 ? "s" : ""}` : "";
    document.getElementById(
      "total-hours"
    ).textContent = `${hourText}${minuteText}`;
...
async function logResults() {
  ...
  if (filledDestinations.length === 0) {
    showAlertModal(
      "🚧 Please enter at least one destination before logging your route."
    );
    return;
  }
...
async function optimizeRoute() {
  ...
  if (currentDestinations.length < 2) {
    showAlertModal(
      "🚧 Please add at least two destinations to optimize the route."
    );
    return;
  }
  ...
  directionsService.route(request, (response, status) => {
    if (status === "OK" && response.routes[0].waypoint_order) {
      ...
      const gasPrice = parseFloat(
        document.getElementById("gas-price").value || 0
      );
...
function addDestination() {
  const newDestinationIndex =
    document.querySelectorAll("#destinations-container .destination").length +
    1;
  const container = document.getElementById("destinations-container");
...
function logEntry(data) {
  ...
  data.profitPerHour =
    totalHours > 0 ? (netProfitFloat / totalHours).toFixed(2) : "0.00";
...
<div><strong>Destinations:</strong><br>
  <div>
    ${(entry.destinations || []).map((dest) => `- ${dest}`).join("<br>")}
      ...
      <strong>Net Profit:</strong> $${parseFloat(entry.netProfit || 0).toFixed(
        2
      )}
    </div>
    <div id="details-${realIndex}" class="hidden"></div>
  </div>
...
function recalculateEditMileageAndCosts() {
  ...
  directionsService.route(request, (response, status) => {
    if (status === "OK") {
      ...
      showAlertModal(
        "⚠️ No route could be found between the entered locations. Please double-check the addresses."
      );
    } else {
...
function renumberEditDestinations() {
  const destinationDivs = document.querySelectorAll(
    "#edit-destinations-container .destination"
  );
    ...
    entry.destinations.forEach((destination, i) => {
      ...
      destDiv.innerHTML = `
        ...
        <input type="text" id="edit-destination-${
          i + 1
        }" list="recent-destinations" value="${destination}">
...
async function saveEditedLogEntry() {
  ...
  entry.profitPerHour =
    sessionHours > 0 ? (netProfitFloat / sessionHours).toFixed(2) : "0.00";
  ...
  reader.onload = async function (e) {
    try {
      ...
      if (successCount === 0) {
        if (duplicateCount > 0) {
          showAlertModal(
            "⚠️ All entries in the file were duplicates and have been skipped."
          );
        } else {
          showAlertModal(
            "❌ The file format appears to be invalid or unsupported."
          );
        }
        return;
      }
...
async function submitAuth() {
  ...
  if (res.ok) {
    ...
    if (isSignup && data.resetKey) {
      showAlertModal(
        `🔐 Your reset key is:<br><br><code>${data.resetKey}</code><br><br>Save this key somewhere safe.`,
        () => location.reload()
      );
    } else {
...
function updateLogSummary(filterFn) {
  ...
  const profitPerHour =
    totalHoursDecimal > 0 ? totalNetProfit / totalHoursDecimal : 0;

  const fullHours = Math.floor(totalHoursDecimal);
  const fullMinutes = Math.round((totalHoursDecimal % 1) * 60);
  const formattedTime = `${fullHours} hour${fullHours !== 1 ? "s" : ""}${
    fullMinutes > 0
  ...
  summaryDiv.innerHTML = `
    <div>
      <div>
        Net Profit: <span>$${totalNetProfit.toFixed(2)}</span>
      </div>
      <div>
        Profit Per Hour: <span>$${profitPerHour.toFixed(2)}</span>
      </div>
      <div>
        Total Hours: <span>${formattedTime}</span>
      </div>
    </div>
  `;
}
...
function toggleLogEntryDetails(index) {
  ...
  <div>${destinations}</div> <!-- second div for addresses -->
...
function renumberMainDestinations() {
  const destinationDivs = document.querySelectorAll(
    "#destinations-container .destination"
  );
...
function exportToCSVWithTotals() {
  ...
  const csvRows = filtered.map((entry) => {
    ...
    const earningsString = `"${(entry.earnings || [])
      .map((e) => `$${parseFloat(e).toFixed(2)}`)
      .join(";")}"`;
  ...
  const totalWorkedMinutes = Math.max(
    totalSessionMinutes - totalDriveMinutes,
    0
  );
  const formattedWorkedTime =
    formatHoursAndMinutesFromMinutes(totalWorkedMinutes);
  const formattedTotalTime =
    formatHoursAndMinutesFromMinutes(totalSessionMinutes);
  const profitPerHour =
    totalSessionMinutes > 0 ? totalNetProfit / (totalSessionMinutes / 60) : 0;
  const formattedDriveTime = `${Math.floor(totalDriveMinutes / 60)} hour${
    Math.floor(totalDriveMinutes / 60) !== 1 ? "s" : ""
  } ${totalDriveMinutes % 60} minutes`;
...
function exportToPDFWithTotals() {
  ...
  const profitPerHour =
    totalSessionMinutes > 0 ? totalNetProfit / (totalSessionMinutes / 60) : 0;
  const formattedDriveTime = `${Math.floor(totalDriveMinutes / 60)} hr ${
...
document.addEventListener("click", function (event) {
  ...
  const accountToggle = document.getElementById(
    "hamburger-manage-account-toggle"
  );
  ...
  if (logMenu?.contains(event.target) || accountMenu?.contains(event.target))
    return;
...
async function handleGoogleSignIn(response) {
  ...
  try {
    const res = await fetch(
      "https://logs.gorouteyourself.com/api/google-login",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
window.addEventListener("scroll", () => {
  ...
  scrollTimeout = setTimeout(() => {
    ...
    topBar.style.boxShadow =
      window.scrollY > 10 ? "0 2px 6px rgba(0,0,0,0.1)" : "none";
  }, 50);
});
...
function installApp() {
  if (deferredPrompt) {
    ...
    alert("⚠️ Installation is not supported or already installed.");
  }
}
