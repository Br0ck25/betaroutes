...
async function loadLog() {
  ...
  try {
    ...
    console.warn(
      "⚠️ Offline or failed to load logs. Loading from local cache.",
      err
    );
...
function updateLogSummary(filterFn) {
  ...
  summaryDiv.innerHTML = `
    <div style="display: flex; flex-direction: column; align-items: center;">
      ...
      <div style="margin-bottom: 8px;">
        Profit Per Hour: <span style="color: blue;">$${profitPerHour.toFixed(
          2
        )}</span>
      </div>
      <div>
...
function resumeDraftTrip() {
  ...
  document.getElementById("total-hours").value = data.hoursWorked || "";
...
function clockInNow() {
  ...
  console.log("📝 Saving this to localStorage:", draft);

  try {
    localStorage.setItem("ongoingTrip", JSON.stringify(draft));
    console.log("✅ ongoingTrip saved!");
  } catch (e) {
    console.error("❌ Failed to save ongoingTrip:", e);
  }

  console.log("🧪 Now reading it back:", localStorage.getItem("ongoingTrip"));
}
