...
window._universalCallback = null;
}
...
console.warn("⚠️ Offline or failed to load logs. Loading from local cache. Error:", err.message);
const cached = localStorage.getItem("cachedLogs");
try {
...
setInterval(saveDraftTrip, 5000);
...
if (originalMileage === null) return;
...
const mpg = parseFloat(document.getElementById("edit-mpg").value) || 1;
const gasPrice =
...
if (successCount === 0) {
if (duplicateCount > 0) {
showAlertModal("⚠️ All entries in the file were duplicates and have been skipped.");
} else {
...
filterLogs();
if (successCount > 0) {
showAlertModal(`✅ Imported ${successCount} entr${successCount === 1 ? "y" : "ies"} successfully.${duplicateCount > 0 ? `\n⚠️ Skipped ${duplicateCount} duplicate${duplicateCount === 1 ? "" : "s"}.` : ""}`);
} else {
showAlertModal("❌ The file format appears to be invalid or unsupported.");
}
} catch (err) {
